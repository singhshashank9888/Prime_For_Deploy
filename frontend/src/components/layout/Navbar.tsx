...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      gsap.fromTo(".mobile-link",
        { opacity: 0, y: 20 },
        { opacity: 1, y: 0, stagger: 0.1, delay: 0.2, duration: 0.4, ease: "power2.out" }
      );
    } else {
  ...
  return (
    <>
      ...
      {/* --- MOBILE MENU OVERLAY - MOVED OUTSIDE NAV TO FIX BLUR CLIPPING --- */}
      <div
        ref={menuRef}
        className={`fixed inset-0 bg-white z-[90] flex flex-col pt-24 px-6 md:px-12 transition-opacity duration-300 lg:hidden overflow-y-auto ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
        style={{ transform: !isOpen ? 'translateX(100%)' : 'none' }}
      >
        <div className="flex flex-col space-y-4 md:space-y-6 pb-12">
          ...
          <div className="mobile-link">
            ...
            <div className={`overflow-hidden transition-all duration-500 ${mobileDeptOpen ? "max-h-[800px] mt-4 opacity-100" : "max-h-0 opacity-0"}`}>
